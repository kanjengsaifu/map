...
<?php
	echo"<div align='right'><a href='mod/printataguna.php?xa=$xa&xb=$xb&xz=$xz'><img src='img/excel.png'></a></div>";
        $no = 1;
       echo "<div class='table-container'>";
        echo "<table id='fixed-table' class='table table-striped table-bordered'>
                ...
                <tr>
					...
					while($r = mysql_fetch_array($s)){
						 echo "<th colspan='2'>$r[namatatagunalahan]</th>";
					}                    
                echo "</tr></thead><tbody>";
        while($d = mysql_fetch_array($sql)){
            echo "<tr>
                    <td>$no</td>
                    <td>$d[nama]</td>";
					$ss = mysql_query("select * from tbtatagunalahan order by id asc");
					while($rr = mysql_fetch_array($ss)){
						$sd = mysql_query("select sum(luas) as jml from infotataguna where tataguna = '$rr[id]' and desa like '$d[id]%'order by id asc");
						$tt=mysql_fetch_array($sd);

						$qn=mysql_query("Select sum(luas) as jmal from infotataguna where desa like '$d[id]%' ");     
		...
		$sss = mysql_query("select * from tbtatagunalahan order by id asc");
		while($rrr = mysql_fetch_array($sss)){
			if($xa == undefined){
				$q=mysql_query("Select sum(luas) as jmil from infotataguna where tataguna ='$rrr[id]'");
			}
			else{
				$q=mysql_query("Select sum(luas) as jmil from infotataguna where tataguna ='$rrr[id]' and desa like '$xa%'");
			}

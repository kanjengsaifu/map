...
$id=$_GET['id'];
 $jnis=$_GET['jnis'];
 $qry = "select * from desa where id LIKE('$id')";
$q=mysql_query($qry);
...
var feature3;
	...
	<?php
			$jk=1;
			$qrt=mysql_query("select * from infostatus where desa like '$id'");
			while($rh=mysql_fetch_array($qrt)){
				$pol=mysql_query("Select * from tbstatuslahan where id='$rh[status]'");

...
<?php   
	...
	$kbupten=strtolower($d[kabupaten]);
	$kcamatan=strtolower($d[kecamatan]);
	
        $KecPurwakarta= file_get_contents("http://peta.klikdesa.com/geojson/$kbupaten/$kcamatan.geojson",null,null); 
...
<head>
  ...
  background-color: #FFFFFF; 
	...
	<script type="text/javascript" src="<?= $base_url ?>js/GeoJSON.js"></script>
...
while($r2=mysql_fetch_array($query2)){ ?>
 case "<? echo"$r2[id]"; ?>":
            gambar_tanda = '<?= $base_url ?>gbrkat/<? echo"$r2[gambar]"; ?>';
            break;
<? } ?>
...
function ambildatabase(akhir){
    if(akhir=="akhir"){
        url = "ambildata.php?akhir=1&username=<? echo"$_GET[id]"; ?>&jnis=<? echo"$jnis"; ?>";
    }else{
        url = "ambildata.php?akhir=0&username=<? echo"$_GET[id]"; ?>&jnis=<? echo"$jnis"; ?>";
    }
    $.ajax({
        url: url,
        dataType: 'json',
...
function setinfo(petak, nomor){
    google.maps.event.addListener(petak, 'click', function() {
		...
		var contentString='<table id=ntable><tr><td colspan=2></td><tr><td><img src=<?= $base_url ?>gbrmarker/medium_'+gambar[nomor]+'></td><td>'+id_infox[nomor]+'</td></tr></table>';
		var infowindow = new google.maps.InfoWindow({
